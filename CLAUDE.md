...
- **Types**: `src/types/note.ts` defines the single `Note` interface used throughout. Note the field intentionally excluded: `tags` is not present and is deliberately left out for a course exercise (see the `❌` comment in that file) — don't add it unless asked.
